<?php
// ============================================================
// SecondChance Mart - Admin Login
// ============================================================
require_once __DIR__ . '/../config/config.php';
require_once __DIR__ . '/../config/database.php';
require_once __DIR__ . '/../includes/functions.php';

if (isLoggedIn() && userRole() === 'admin') redirect(SITE_URL . '/admin/dashboard.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email && $password) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            $_SESSION['role']    = 'admin';
            $a = $pdo->prepare("SELECT name FROM admins WHERE user_id=?");
            $a->execute([$user['id']]);
            $_SESSION['name'] = $a->fetchColumn() ?: 'Admin';
...
<body>
<div class="auth-page" style="background:linear-gradient(135deg,#1a1a2e,#16213e);">
    <div class="auth-card" style="max-width:400px;">
        <div class="auth-header" style="background:linear-gradient(135deg,#1a2a1a,#1e4d2b);">
            <div class="auth-logo">🔑</div>
            <h3>Admin Portal</h3>
            <p class="mb-0 opacity-75 small"><?= SITE_NAME ?> Administration</p>
        </div>
        <div class="auth-body">
            <?php if ($error): ?>
